...
* Here we can see that the number of low mileage car is bigger in our data set. It is almost twice the amount of high mileage car.

**Next, lets look at the price variation in these two group.**
...
* There is not much variation in price both high mileage and low mileage car. We will need to investigate more if the mean price for high mileage car is different from mean price for low mileage car.
...
> Is there a difference between the true underlying average price of high mileage car and low mileage car?
...
> There is a strong evidence that the true average price for the high mileage car is lower than low mileage car. This is because the p-value we got here is smaller than the significance level 5% thus we are able to reject null hypothesis.
...
* The seems to be a very weak correlation between mileage and price here. We can see the blue line is almost a horizontal line here.
...
* The interpretation of the coefficient of Beta_1 here is that for every 1 mile increase in car mileage, the expected price of the car decrease by EUR 0.03.

### Checking the assumptions

* Now lets look at out assumptions.
...
**We have not yet covered it in class at the moment, will be included in the final report**
...
* For question 2(prediction of car prices), our result might not be valid since it violates the normal errors assumption. Also as you can see from the correlation graph, the amount of information about to predict prices from mileage is not very useful. This is because the strength of correlation between price and mileage here is close to 0 (horizontal). Therefore making it harder to precisely predict car prices just based on mileage.

## Shortcomings

* One shortcoming is that there might be a better way to classified high mileage car and low mileage car. For example, a car that was used for 1 year with 50000km mileage can be considered as high mileage car since the owner used it excessively in just one year period. However, the data does not have information about how many years was it being used.

## Future directions

* One thing that can be done in the future for our project is to construct a prices model controlling for several factors. We feel that this problem is more suitable for multiple linear regression (outside of this course concept)

* Also, instead of taking data just from one car dealership, it is better to aggregate the data from several used car dealership. However, the data might not be standardized and there is a high change of lots of missing data. Data imputation(outside of this course concept) might be useful here.

## Conclusion

* `Question 1:` There is a strong evidence that the price of high mileage car is lower than low mileage car.

* `Question2:` Our model is not the best for predicting prices of used car. This is because we are not able to accurately predict prices just based on the mileage in kilometres.
